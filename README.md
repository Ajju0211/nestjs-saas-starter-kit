...
| Multi-Tenancy   | Optional (Prisma RLS + TenantGuard) |
...
│   │   ├── tenant.schema.prisma       # Tenant (Company) model for multi-tenancy
...
HTTP Request
  ...
  │     └── TenantGuard    — enforces @RequireTenant(); injects tenantId into context
  │
...
The template supports **optional, strict Multi-Tenancy** out of the box, functioning like Row-Level Security (RLS) but implemented at the application ORM layer.

**How it works:**
1. **Schema:** The database contains a `Tenant` model. Users, Uploads, and AuditLogs have an optional `tenantId` relationship.
2. **JWT:** Upon login, if a user belongs to a tenant, the `tenantId` is baked into their JWT payload.
3. **Guard & Context:** The global `TenantGuard` extracts this `tenantId` and patches it into the `RequestContext`. Controllers can strictly require a tenant by adding the `@RequireTenant()` decorator.
4. **Data Isolation (The Magic):** A Prisma Client Extension intercepts all database queries (e.g. `findMany`, `update`, `delete`). If a `tenantId` exists in the context, it automatically forces a `WHERE tenantId = ...` filter into the query. Developers never have to manually filter by tenant ID!

> **Note:** This feature is 100% optional. If you don't assign tenants to users and don't use the `@RequireTenant()` decorator, the system runs as a pure RBAC application without any side effects. Global `SUPER_ADMIN`s can also bypass this or impersonate tenants via the `x-tenant-id` HTTP header.
