...
export class AuthService {
  ...
  async register(dto: RegisterDto): Promise<AuthResponse> {
    ...
    const tokens = await this.generateTokens(user.id, user.email, user.role, user.tenantId || undefined);
   ...
   */
  private async generateTokens(userId: string, email: string, role: string, tenantId?: string): Promise<AuthTokens> {
    const payload: JwtPayload = { sub: userId, email, role: role as never, tenantId };
